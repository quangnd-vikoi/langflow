...
const McpServerTab = ({ folderName }: { folderName: string }) => {
	...
	const [selectedPlatform, setSelectedPlatform] = useState(
		operatingSystemTabs.find((tab) => tab.name.includes(getOS() || "windows"))?.name
	);

	const isAutoLogin = useAuthStore((state) => state.autoLogin);
	const isAuthApiKey = ENABLE_MCP_COMPOSER ? currentAuthSettings?.auth_type === "apikey" : !isAutoLogin;

	// Check if the current connection is local
	const isLocalConnection = useCustomIsLocalConnection();

	const [selectedMode, setSelectedMode] = useState(isLocalConnection ? "Auto install" : "JSON");
	...
	const handleOnNewValue = (value: any) => {
		...
		const finalAuthSettings = ENABLE_MCP_COMPOSER ? currentAuthSettings : { auth_type: "none" };
  ...
  "mcpServers": {
	...
	const MCP_SERVER_DEPLOY_TUTORIAL_LINK = "https://docs.langflow.org/mcp-server";
	...
	const hasAuthentication = currentAuthSettings?.auth_type && currentAuthSettings.auth_type !== "none";
							...
							<span className="flex gap-2 items-center">
								...
								{!hasAuthentication ? (
									...
									<span className="text-accent-emerald-foreground flex gap-2 text-mmd items-center">
										<ForwardedIconComponent name="Check" className="h-4 w-4 shrink-0" />
										{AUTH_METHODS[
						...
						<div className="flex flex-col gap-1">
							...
							{autoInstallers.map((installer) => (
								<Button
									...
									disabled={loadingMCP.includes(installer.name) || !isLocalConnection}
									onClick={() => {
