...
export const HeaderButtons = ({
	...
	const [isDismissedDialog, setIsDismissedDialog] = useState(userData?.optins?.dialog_dismissed);
	const [isGithubStarred, setIsGithubStarred] = useState(userData?.optins?.github_starred);
	const [isDiscordJoined, setIsDiscordJoined] = useState(userData?.optins?.discord_clicked);
          ...
          /> */}
			...
			<div className="flex shrink-0 items-center justify-between gap-2 pt-2">
				...
				<div className="flex items-center gap-1">
					<UploadFolderButton onClick={handleUploadFlowsToFolder} disabled={isUpdatingFolder} />
					<AddFolderButton onClick={addNewFolder} disabled={isUpdatingFolder} loading={isPending} />
				</div>
			</div>
		</>
	);
